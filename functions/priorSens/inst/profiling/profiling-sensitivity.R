...
integrate(f=inla.dmarginal, lower=72, upper=80, marginal = inla_marginal_posterior)
simp13rule_matrix(density_matrix = inla_marginal_posterior)
...
replicate(n=50, compute_sensitivity(grid = gaussian_grid_polar,
...
Rprof(file.path(root, "test1_profile"))
replicate(n=500, compute_corrected_posteriors(prior_type = "gaussian", 
                                              hyperpar_new = c(70.1, 0.51), 
                                              hyperpar_old = c(70, 0.5),
                                              inla_marginal_posterior = inla_marginal_posterior, 
                                              integration_limits = TRUE))
Rprof(NULL)
summaryRprof(file.path(root, "test1_profile"))
...
Rprof(file.path(root, "test1_profile"))
replicate(n=500, compute_hellinger_distance_inla(inla_posterior_1 = inla_marginal_posterior, 
                                inla_posterior_2 = inla_marginal_posterior, 
                                integration_limits = TRUE, 
                                renormalize = TRUE))
Rprof(NULL)
summaryRprof(file.path(root, "test1_profile"))
